...
Page({
    ...
    openWhyList(e){//打开取消订单原因列表选择弹窗
        ...
        if(this.data.orderInfo.activityOrderRefundResponse){
    ...
    getOrderDetailInfo(){
        app.sjrequest1('/activityOrderBusiness/detailOrder',{
            ...
            if(res.statusCode==200 && res.data.code===0){
                ...
                this.setData({
                    orderInfo:data,shippingAddress,
                    fullAddress,joinGroupUser,groupInfo
                },()=>{
                    if(data.expressNo){this.getLogisticsInfo();}
                });

                this.computedSurplus(data.deliveryTime);//计算剩余收货时间
                this.computedGroupEndTime(groupInfo.overTime);
            }
        });
    },
    ...
    computedSurplus(deliveryTime){
        if(deliveryTime){
            var totalTimes=this.data.residueDay*86400000;
            var nowTimes=new Date().getTime();
    ...
    computedGroupEndTime(endDate){//计时拼团结束倒计时毫秒数
        var nowTimes=new Date().getTime();
        var endTimes=this._parseDate(endDate || '','number');
        var diffTimes=endTimes-nowTimes;
        this.setData({groupEndTimes:diffTimes});
    },
