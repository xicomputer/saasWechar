...
Component({
     ...
     */
    methods: {
        darwRoundRect(x, y, w, h, r,bgcolor, ctx) {//绘制椭圆
            ctx.save()
            ctx.beginPath()
        ...
        syntheticImg(imgs){
            ...
            const ctx = wx.createSelectorQuery().in(this).select(canvasId).fields({
                node: true,
                size: true,
              })
              .exec((res) => {
            ...
            //绘制圆角矩形
            var rectInfo = this.properties.rectInfo;
            if(rectInfo){
